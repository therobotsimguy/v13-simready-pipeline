...
| F45 | Collision | Two sibling movables (both jointed to the same chassis) pass through each other | PhysX auto-skips collision between directly-adjacent articulation links; siblings jointed to the same chassis are NON-adjacent to each other but `PhysxArticulationAPI.EnabledSelfCollisions` defaults to False, so sibling pairs (drawer↔drawer) silently tunnel | apply_physics sets `PhysxArticulationAPI.EnabledSelfCollisions` **conditionally (F45 v2, 2026-04-20)**: enabled for drawer/door assets, **disabled when the articulation contains any `_bracket` body** (2-DOF swivel-caster signature). Rationale: SurgicalChair_A01_01 flew apart at runtime with F45 on because PhysX broadphase produced NaN contact normals on bracket↔wheel hull overlaps — the filterPairs authored by `author_filter_pairs_for_overlapping_siblings` (F66) weren't enough because the NaN comes from hull-level intersection, not bbox-level. Surfaced on MedicalutilityCart_A03_01 adjacent drawers (2026-04-18), refined for 2-DOF casters on SurgicalChair_A01_01 (2026-04-20) |
...
| F64d | Wheel/Collision | After F64c runs, chassis has ZERO colliders — wheels still roll but chassis doesn't block obstacles at its own body level | F64c's strip_chassis_floor_blockers matches meshes whose bottom-Z sits >5mm below wheel cylinder bottoms; on star-base furniture (surgical stool, office chair) every leg of the base legitimately extends to the floor — ALL chassis colliders match, the strip leaves body with 0 colliders, F35 then FAILs C2 | `strip_chassis_floor_blockers` collects candidate meshes in a first pass and, if the candidate count equals the total body-collider count, drops the candidate with the highest zmin (closest to the wheels) from the strip set. Chassis keeps at least one collider; lowest-Z leg meshes still get stripped so wheel cylinders contact ground first. Surfaced on SurgicalChair_A01_01 (2026-04-20): 5-caster star base with leg_body1/leg_body2/leg_heightadjuster all at floor level |
| F64e | Wheel/Collision | F64 cube-wheel audit false-positives on 2-DOF swivel-caster brackets (revolute Z joint, ±9999° limits, naturally cube-shaped housing) | is_continuous heuristic treats any revolute joint with limit span > 1000° as continuous → F64 audit iterates those and flags the bracket body as a non-rolling wheel. But the bracket IS the caster hub; the tire is a separate body jointed to the bracket. Flagging the bracket is the wrong signal. | F64 audit skips any joint whose body1_path or name contains `_bracket` — same pattern as the existing C5 wheel-split-leak exclusion at make_simready.py:474. Only actual wheel-body continuous joints are evaluated for F64. Surfaced on SurgicalChair_A01_01 (2026-04-20): 5 `wheel*_bracket` Xforms falsely flagged as cube-wheels |
| F65 | Collision/Classification | Swivel seat classified as `movable:continuous` gets a primitive Cylinder collider (synthesized by apply_collision_wheels) instead of its real cushion mesh; downstream F64c then strips chassis colliders relative to the bogus cylinder bottom | apply_physics wheel-vs-body dispatch at line 3706 used `is_wheel = info["joint"] == "continuous"` — treats any continuous-joint part as a wheel regardless of geometry. Swivel seats, rotary knobs, turntables all rotate freely (hence continuous) but are not tires; forcing a synthesized cylinder over them cascades into F64b/F64c/F35 failures. | Gate the wheel-path dispatch on BOTH `joint=="continuous"` AND a wheel-name heuristic: part name must contain `wheel`/`caster`/`tire`/`roller`. Non-wheel continuous joints fall through to the standard `apply_collision_q1` mesh-collision path. C2 audit adds a post-hoc detector (any body with a primitive Cylinder collider whose name lacks wheel keywords → F65). Surfaced on SurgicalChair_A01_01 (2026-04-20): seat_01 `movable:continuous` forced the retry workaround `seat=revolute` to sidestep the wheel-cylinder synthesis |
| F66 | Collision | Asset audits 7/7, spawns visually correct, then PhysX crashes with `Illegal BroadPhaseUpdateData` and every rigid body reports Invalid transform at first non-adjacent contact | With F45 `EnabledSelfCollisions=True` on the articulation root, PhysX attempts collision between rigid bodies that are NOT directly joint-connected (separated by ≥2 joints). If any such pair has rest-pose bbox overlap — e.g. a 5-caster chair where the chassis leg body bbox envelops each wheel body's bbox because they share XY footprint — the overlapping convex hulls produce NaN contact normals at first runtime contact, which cascade through the broadphase. Audit's rest-pose checks only inspect joint anchors and per-body collider presence; they do not flag non-adjacent bbox overlaps. | C2 audit walks the joint graph to compute adjacency distance between rigid body pairs (bodies connected by a single joint are distance=1, separated by 2 joints are distance=2, etc.). For every pair at distance ≥ 2 whose world-space bboxes overlap under EnabledSelfCollisions=True, FAIL C2 and cite F66 with example. `apply_physics` also authors `physxCollision:filteredPairs` on one side of each pair via `author_filter_pairs_for_overlapping_siblings` so PhysX explicitly skips them. **Note (2026-04-20):** filterPairs alone wasn't sufficient on SurgicalChair_A01_01 — PhysX still produced NaN at bracket↔wheel hull level. F45 v2 now disables EnabledSelfCollisions entirely for 2-DOF caster articulations, making F66 moot for that asset class. Surfaced on SurgicalChair_A01_01 (2026-04-20): 5 leg↔wheel bbox overlaps, broadphase crashed at ~25s runtime |
| F67 | Hierarchy | Visual mesh stays at rest pose while physics body moves (Fabric view shows chair disappear, USD view shows it in place); ArticulationCfg spawn produces visual/physics divergence | Raw USD authors a redundant nested-duplicate Xform layer (`/outer/same_name` inside `/outer`, both named identically). The classifier picks a grandchild prim as the chassis body (e.g. `leg_01`) while `ArticulationRootAPI` sits on `outer` and the inner Xform becomes an empty pass-through wrapper with no RigidBodyAPI. Isaac Lab's `ArticulationCfg` + Fabric runtime composes the articulation transform chain inconsistently through the empty wrapper → physics updates the chassis world pose but the visual mesh transforms (children of the inner Xform) don't get propagated to Fabric, so render uses the static USD pose and physics uses the updated pose. | `flatten_redundant_xform_layers` (pipeline follow-up) detects the pattern — Xform with `ArticulationRootAPI`, one child Xform with the same name carrying only identity/wrapper xformOps, and a grandchild with `RigidBodyAPI` — and promotes the grandchild's rigid body APIs + world transform up into the inner Xform, flattening the redundant layer. **Must author decomposed `xformOp:translate + xformOp:rotateXYZ + xformOp:scale` on the inner Xform, NOT `xformOp:transform` (single matrix op)** — Isaac Lab's ArticulationCfg / Fabric parse the decomposed form correctly; the matrix form on the chassis body produces articulation binding issues at spawn (caster revolute joints don't fully constrain translation). Matches the trolley/table pattern where the inner Xform IS the chassis body with decomposed xformOps. C4 audit check (future): flag articulation-root prim whose direct-child Xform has no RigidBody but has a grandchild with one. Surfaced on SurgicalChair_A01_01 (2026-04-20): fixed in-place via `/tmp/fix_chair.py` + xformOp decomposition patch; pipeline integration pending |
| F68 | Wheel/Hierarchy | 2-DOF caster bracket visually detaches from its chassis leg during shift-drag teleop — the swivel pivot point "comes off" instead of rotating in place. Only affects casters whose mount mesh in the raw USD carries a non-identity Z rotation (splayed star base pointing each leg outward). | Raw USD authors each splayed caster's mount mesh with a Z rotation facing its leg (e.g. SurgicalChair_A01_01: wheel3 at +75°, wheel5 at -70°; wheel1/2/4 at identity). `split_wheel_structural_parts` reparents the mount into a new `_bracket` Xform using `reparent_prims_preserve_world_xform`, which keeps world pose exact but leaves the mount's own `xformOp:rotateXYZ` intact. Bracket Xform sits at identity; revolute Z swivel joint correctly uses body0-local-Z = world-Z. But the rotated mount shifts the tire/hub mass distribution laterally relative to the bracket origin — every Z swivel has a moment arm equal to the lateral CoM offset, and the ~0.3 kg bracket can't resist the solver impulse. Under drag force the constraint exceeds iteration budget and the bracket visually separates. Surfaced on SurgicalChair_A01_01 (2026-04-20); absent on trolleys/beds/SurgicalTable because those are either 1-DOF (no bracket) or 2-DOF with orthogonal (0°/90°) mount geometry. | `normalize_caster_bracket_mount_rotation` in make_simready.py bakes the rotation into each bracket-child's descendant mesh vertex data using F43's snapshot-reset-reauthor pattern: A) snapshot world points of every Mesh under the rotated child, B) reset the child's rotate ops (TypeRotate{X,Y,Z,XYZ,XZY,…}, TypeOrient) to identity OR — critically — replace any `TypeTransform` 4×4 matrix with a translate-only matrix (because `reparent_prims_preserve_world_xform` collapses the original rotateXYZ chain into a single matrix op, which the first cut of F68 missed), C) recompute the child's L2W with rotations stripped and reauthor mesh local points = new_w2l · world_points. World geometry preserved exactly; bracket-local frame now matches world so tire CoM sits on the swivel axis. Called at the end of `split_wheel_structural_parts` once per bracket. C5 audit walks every `_bracket`-suffixed prim and FAILs if any direct child carries >5° on a rotate op OR a non-identity upper-3×3 in a TypeTransform matrix, citing the fix function by name. Verified 2026-04-21 on SurgicalChair_A01_01 rebuild: pre-fix audit FAILs with `F68: 2 caster bracket child(ren) carry residual rotation >5° (e.g. …wheel3_mount_01 at 75.0°)`, post-fix PASS, patched USD saved. |
| F69 | Limits/Drive | 2-DOF caster asset visually falls apart during shift-drag teleop — bracket detaches from chassis anchor AND the chair body jitters. F68 mount-rotation fix alone does not resolve this; post-F68 joint anchors are 0.00mm mismatched (authoring is correct) yet the articulation still explodes at runtime. Differentiator: InstrumentTrolley_B01_01 (1-DOF casters, damping=2.0 on tire joints) is stable; SurgicalChair_A01_01 (2-DOF casters, damping=2.0 on bracket swivel + 2.0 on tire roll) is not. | The revolute Z bracket swivel joint inherits the door-hinge damping default of 2.0 N·m·s/rad. For a single heavy door that's plenty. For a 2-DOF caster the joint sits in the middle of a chassis(~7 kg) → bracket(~0.3 kg) → tire(~0.3 kg) mass chain where bracket:tire ≈ 1:1. Any lateral chassis impulse propagates down the chain and drives a resonant oscillation — the tire's inertia whips the bracket around its swivel axis faster than the 2.0 dashpot absorbs, the joint solver iteration budget blows, and the constraint visually separates. Real caster bearings have 5–20 N·m·s/rad of rotational damping (grease, needle rollers, plastic bushing); V13 was modeling them as near-frictionless pivots. Root cause = model error, not authoring error. | `make_revolute_joint` caster-swivel branch (`hinge_edge="continuous"`) now authors `drive:angular:damping = 15.0` instead of the door default 2.0. Physically grounded in real bearing friction; empirically kills the resonant whip without locking the swivel so shift-drag still produces natural caster re-alignment. `make_continuous_joint` for tire roll keeps damping=2.0 (trolley confirms that's enough for a 1-DOF roll chain directly attached to chassis). C6 audit walks every `_bracket_joint` revolute and FAILs if `drive:angular:damping < 5.0`, citing the fix. Surfaced on SurgicalChair_A01_01 (2026-04-21) after F68 mount-rotation fix did not resolve detach. |
